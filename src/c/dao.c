...
static int clock_gettime(int clk_id, struct timespec* t)
{
    ...
    int64_t hundredNanos = (int64_t)currentTime.dwLowDateTime | (int64_t)currentTime.dwHighDateTime << 32;
...
int_fast8_t daoShmShm2Img(const char* name, IMAGE* image)
{
    ...
    shmFd = CreateFileW(
        wideShmName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_TEMPORARY, NULL);
 ...
 */
int_fast8_t daoShmImagePart2Shm(
    char* im, uint32_t nbVal, IMAGE* image, uint32_t position, uint16_t packetId, uint16_t packetTotal, uint64_t frameNumber)
{
    daoTrace("\n");
 ...
 */
int_fast8_t
daoShmImageCreate_FIFO(IMAGE* image, const char* name, long naxis, uint32_t* size, uint8_t atype, int shared, int NBkw, uint32_t fifo_size)
{
    daoTrace("\n");
    long i; //,ii;
    long nelement;
    ...
    if (shared == 1)
    {
        ...
        // run, or size changed).
        shmFm = CreateFileMapping(shmFd, NULL, PAGE_READWRITE, (DWORD)(sharedsize >> 32), (DWORD)sharedsize, NULL);
        if (shmFm == NULL)
        {
            CloseHandle(shmFd);
            daoError("Error creating file mapping (%s)\n", shmName);
    ...
    if (atype == _DATATYPE_UINT8)
    {
        ...
        if (image->array.UI8 == NULL)
        {
            ...
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "Requested memory size = %ld elements * %ld buffers = %f Mb\n",
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(uint8_t));
    ...
    if (atype == _DATATYPE_INT8)
    {
        ...
        if (image->array.SI8 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(int8_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_UINT16)
    {
        ...
        if (image->array.UI16 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(uint16_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_INT16)
    {
        ...
        if (image->array.SI16 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(int16_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_UINT32)
    {
        ...
        if (image->array.UI32 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(uint32_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_INT32)
    {
        ...
        if (image->array.SI32 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(int32_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_UINT64)
    {
        ...
        if (image->array.SI64 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(uint64_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_INT64)
    {
        ...
        if (image->array.SI64 == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(int64_t));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_FLOAT)
    {
        ...
        if (image->array.F == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(float));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_DOUBLE)
    {
        ...
        if (image->array.D == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(double));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_COMPLEX_FLOAT)
    {
        ...
        if (image->array.CF == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(complex_float));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
    ...
    if (atype == _DATATYPE_COMPLEX_DOUBLE)
    {
        ...
        if (image->array.CD == NULL)
        {
            ...
            fprintf(stderr,
                    ...
                    (long)nelement,
                    (long)fifo_size,
                    1.0 / 1024 / 1024 * nelement * fifo_size * sizeof(complex_double));
            fprintf(stderr, " %c[%d;m", (char)27, 0);
            exit(0);
        }
    }
...
int_fast8_t daoShmCombineShm2Shm(IMAGE** imageCube, IMAGE* image, int nbChannel, int nbVal)
{
    ...
    if (image->md[0].atype == _DATATYPE_UINT8)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            {
                image[0].array.UI8[fifo_writing_offset + pp] += imageCube[k][0].array.UI8[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_INT8)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.SI8[fifo_writing_offset + pp] += imageCube[k][0].array.SI8[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_UINT16)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.UI16[fifo_writing_offset + pp] += imageCube[k][0].array.UI16[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_INT16)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.SI16[fifo_writing_offset + pp] += imageCube[k][0].array.SI16[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_UINT32)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.UI32[fifo_writing_offset + pp] += imageCube[k][0].array.UI32[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_INT32)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.SI32[fifo_writing_offset + pp] += imageCube[k][0].array.SI32[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_UINT64)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.UI64[fifo_writing_offset + pp] += imageCube[k][0].array.UI64[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_INT64)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.SI64[fifo_writing_offset + pp] += imageCube[k][0].array.SI64[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_FLOAT)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.F[fifo_writing_offset + pp] += imageCube[k][0].array.F[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_DOUBLE)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.D[fifo_writing_offset + pp] += imageCube[k][0].array.D[fifo_reading_offset[k] + pp];
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_COMPLEX_FLOAT)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.CF[fifo_writing_offset + pp].re += imageCube[k][0].array.CF[fifo_reading_offset[k] + pp].re;
                image[0].array.CF[fifo_writing_offset + pp].im += imageCube[k][0].array.CF[fifo_reading_offset[k] + pp].im;
            }
        }
    }
    else if (image->md[0].atype == _DATATYPE_COMPLEX_DOUBLE)
    {
        for (pp = 0; pp < nbVal; pp++)
        {
            ...
            for (k = 0; k < nbChannel; k++)
            {
                image[0].array.CD[fifo_writing_offset + pp].re += imageCube[k][0].array.CD[fifo_reading_offset[k] + pp].re;
                image[0].array.CD[fifo_writing_offset + pp].im += imageCube[k][0].array.CD[fifo_reading_offset[k] + pp].im;
            }
        }
    }
...
int_fast8_t daoShmWaitForSemaphoreTimeout(IMAGE* image, int32_t semNb, const struct timespec* timeout)
{
    ...
    DWORD millis = (timeout->tv_sec - now.tv_sec) * 1000 + (timeout->tv_nsec - now.tv_nsec) / 1000000;

    if ((int)millis < 0)
        millis = 0; // prevent underflow
...
int_fast8_t daoShmTimestampShm(IMAGE* image)
{
    ...
    uint32_t fifo_prior_write = (fifo_last_written == 0) ? vol_md[0].fifo_size - 1 : fifo_last_written - 1;
