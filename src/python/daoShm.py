...
class TIMESPECFIXED(ctypes.Structure):
    _fields_ = [("firstlong", ctypes.c_int64), ("secondlong", ctypes.c_int64)]
...
if sys.platform == "darwin":
    ...
    class IMAGE_METADATA(ctypes.Structure):
        class ATIME(ctypes.Union):
            _fields_ = [("ts", timespec), ("tsfixed", TIMESPECFIXED)]
...
class shm:
    ...
    def __init__(self, fname=None, data=None, nbkw=0, pubPort=5555, subPort=5555, subHost="localhost", logLevel=0, depth=1):
        ...
        self.daoShmShm2Img.argtypes = [ctypes.c_char_p, ctypes.POINTER(IMAGE)]
        ...
        self.daoShmImage2Shm.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.POINTER(IMAGE)]
        ...
        self.daoShmImage2ShmQuiet.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.POINTER(IMAGE)]
        ...
        self.daoShmCombineShm2Shm.argtypes = [ctypes.POINTER(ctypes.POINTER(IMAGE)), ctypes.POINTER(IMAGE), ctypes.c_int, ctypes.c_int]
        ...
        self.daoShmWaitForSemaphore.argtypes = [ctypes.POINTER(IMAGE), ctypes.c_int32]
        self.daoShmWaitForSemaphore.restype = ctypes.c_int8

        self.daoShmWaitForSemaphoreTimeout = daoLib.daoShmWaitForSemaphoreTimeout
        self.daoShmWaitForSemaphoreTimeout.argtypes = [ctypes.POINTER(IMAGE), ctypes.c_int32, ctypes.POINTER(timespec)]
        ...
        self.daoShmGetNextSegment.argtypes = [
            ctypes.POINTER(IMAGE),
        ...
        self.daoShmGetNewestSegment.argtypes = [
            ctypes.POINTER(IMAGE),
            ctypes.POINTER(ctypes.c_void_p),
            ctypes.POINTER(ctypes.c_uint32),
            ctypes.POINTER(ctypes.c_uint64),
        ...
        if fname == "":
            ...
            log.info(f"{fname} will be created or overwritten")
            dataSize = data.shape
            self.daoShmImageCreate_FIFO(
            ...
            _result = self.daoShmImage2Shm(cData, nbVal, ctypes.byref(self.image))
        else:
            # log.info("loading existing %s " % (fname))
            _result = self.daoShmShm2Img(fname.encode("utf-8"), ctypes.byref(self.image))
        # Publisher
    ...
    def get_data_next(self, wait=False, reform=True):
        ...
        result = self.daoShmGetNextSegment(ctypes.byref(self.image), ctypes.byref(arrayPtr), ctypes.byref(temp32), ctypes.byref(temp64))

        data = None

        if result == self.DAO_SUCCESS or result == self.DAO_OVERWRITE:
            arraySize = np.ctypeslib.as_array(ctypes.cast(self.image.md.contents.size, ctypes.POINTER(ctypes.c_uint32)), shape=(3,))

            arrayPtr = ctypes.cast(arrayPtr, ctypes.POINTER(daoType2CtypesType(self.image.md.contents.atype)))
            ...
            if arraySize[2] == 0:
                if arraySize[1] == 0:
                    ...
                    data = np.ctypeslib.as_array(
                        arrayPtr, shape=(arraySize[0], arraySize[1])
                    )  # .astype(daoType2NpType(self.image.md.contents.atype))
            else:
                data = np.ctypeslib.as_array(
                    arrayPtr, shape=(arraySize[0], arraySize[1], arraySize[2])
                )  # .astype(daoType2NpType(self.image.md.contents.atype))
    ...
    def get_data_arbitrary(self, index):
        ...
        _result = self.daoShmGetArbitrarySegment(ctypes.byref(self.image), ctypes.byref(arrayPtr), ctypes.c_uint32(fifo_idx))

        # Cast our void pointer to the desired type
        arrayPtr = ctypes.cast(arrayPtr.value, ctypes.POINTER(daoType2CtypesType(self.image.md.contents.atype)))
    ...
    def get_data(self, check=False, reform=True, semNb=0, timeout=0, spin=False):
        ...
        result = self.daoShmGetNewestSegment(
            ctypes.byref(self.image), ctypes.byref(arrayPtr), ctypes.byref(seg_idx), ctypes.byref(seg_cnt0)
        )
    ...
    def get_history(self, num_items=1, check=False, semNb=0, timeout=0, spin=False, buffer=False):
        ...
        for idx_offset in range(num_items):
            ...
            if current_data.dtype.fields is not None and "real" in current_data.dtype.fields and "imag" in current_data.dtype.fields:
                # Reconstruct complex array by combining real and imaginary parts
    ...
    def get_meta_data(self, index=None):
        ...
        if index is None:
            ...
            self.daoShmGetNewestSegment(ctypes.byref(self.image), ctypes.byref(seg_ptr), ctypes.byref(seg_idx), ctypes.byref(seg_cnt0))
        else:
    ...
    def reset_tail(
        ...
        result = self.daoShmResetTail(ctypes.byref(self.image), ctypes.byref(tail_index), ctypes.byref(tail_timestamp))
        return result
    ...
    def subscribe(self):
        ...
        while True:
            if self.subEnable:
                try:
                    _topic = self.subSocket.recv_string()  # drain ZMQ envelope frame
    ...
    def close(self):
        ...
        --------------------------------------------------------------"""
        _result = self.daoShmCloseShm(ctypes.byref(self.image))
