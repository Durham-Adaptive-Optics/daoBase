...
#define SEMAPHORE_MAXVAL                                                                                                                   \
    1 /**< maximum value for each of the semaphore, mitigates warm-up time when processes catch up with data that has accumulated */
...
#define CDtype 11 /**< default data type for complex */
     ...
     * The array can be 1D (list of events), or 3D (N x 1 x M) for a circular buffer where the z-index (slice) is incremented between each
     ...
     * - [1] create IMAGE structure type EVENT_UI8_UI8_UI16_UI8. Size n x 1 x m, where n = max # of event per "exposure", m = number of
     * slices in circular buffer. Note that md[0].size[0]=m, md[0].size[1]=1, md[0].size[2]=m
     ...
     * - [7] When "exposure" completed, set md[0].atime to exposure time start (see step [5]), md[0].cnt1=k (last slice written),
     * md[0].cnt2=ii (number of events), set md[0].write=0 (write completed), increment md[0].cnt0, and post all semaphores
     * - [8] Increment k (if k=md[0].size[2], set k=0), return to step [4]
     *
     * @warning Array size will define the maximum number of events packed in IMAGE. User is responsible for pushing out IMAGE and starting
     * a new IMAGE or slice when max number of events is reached.
     *
     */
    typedef struct
    {
    ...
    typedef struct
    {
         ...
         *    long int tv_nsec
         * This is the rest of the elapsed time (a fraction of a second), represented as the number of nanoseconds. It is always less than
         * one billion.
         *
         * On (most ?) 64-bit systems:
         * sizeof(struct timespec) = 16 :  sizeof(long int) = 8;  sizeof(time_t) = 8
         *
         * @warning sizeof(struct timespec) is implementation-specific, and could be smaller that 16 byte. Users may need to create and
         * manage their own timespec implementation if data needs to be portable across machines.
         */
        union
        {
            struct timespec ts;
    ...
    typedef struct /**< structure used to store data arrays                      */
    {
         ...
         *
         * For 3D image with pixel indices ii (x-axis), jj (y-axis) and kk (z-axis), the pixel values are stored as array.<TYPE>[ kk *
         * md[0].size[1] * md[0].size[0] + jj * md[0].size[0] + ii ] \n image md[0].size[0] is x-axis size, md[0].size[1] is y-axis size,
         * md[0].size[2] is z-axis size
         *
    ...
    int_fast8_t daoShmImagePart2Shm(
        char* procim, uint32_t nbVal, IMAGE* image, uint32_t position, uint16_t packetId, uint16_t packetTotal, uint64_t frameNumber);
