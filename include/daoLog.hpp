...
namespace Dao
{
    ...
    namespace Log
    {
        ...
        const std::map<LEVEL, std::string> LEVEL_TEXT = {{LEVEL::TRACE, "[TRACE]   "},
                                                         {LEVEL::DEBUG, "[DEBUG]   "},
                                                         {LEVEL::INFO, "[INFO ]   "},
        ...
        class NetworkLog
        {
            ...
            void dump_zmq_error(int error)
            {
                ...
                {
                case EAGAIN:
                    std::cout << error << ": " << "Non-blocking mode was requested and the message cannot be sent at the moment."
                              << std::endl;
                    break;
                case ENOTSUP:
                ...
                case EFSM:
                    std::cout << error << ": "
                              << "The zmq_send() operation cannot be performed on this socket at the moment due to the socket not being in "
                                 "the appropriate state. This error may occur with socket types that switch between several states, such "
                                 "as ZMQ_REP. See the messaging patterns section of zmq_socket(3) for more information"
                              << std::endl;
                    break;
                case ETERM:
                ...
                case ENOTSOCK:
                    ...
                    break;
                case EINTR:
                    std::cout << error << ": " << "The operation was interrupted by delivery of a signal before the message was sent."
                              << std::endl;
                    break;
                case EFAULT:
            ...
            const std::map<LEVEL, Dao::LogMessage::log_level> m_level_text = {{LEVEL::TRACE, Dao::LogMessage::TRACE},
        ...
        class Logger
        {
            ...
            std::string construct_string(LOG_MESSAGE& message)
            {
                ...
                tmpString << message.comp_name << ":" << message.timestamp << " " << LEVEL_TEXT.at(message.level) << " - "
                          << message.message;
